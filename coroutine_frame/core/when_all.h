...
// TRUE PARALLEL VERSION: All tasks start concurrently, event-driven (no
// polling)

namespace detail {
// Shared state for coordinating multiple tasks
template <typename T> struct when_all_state {
    ...
    when_all_state(size_t n) : total(n) { results.resize(n); }
...
template <typename T>
task<void> when_all_task(task<T> t, std::shared_ptr<when_all_state<T>> state,
                         size_t index) {
    try {
