...
template <typename T> task<std::optional<T>> try_task(task<T> t) {
    try {
...
template <typename T> task<T> catch_and_log(task<T> t, const char *task_name) {
    try {
        ...
        std::fprintf(stderr,
                     "[ERROR] Task '%s' failed with unknown exception\n",
                     task_name);
        throw;
    }
}
...
auto retry(TaskFactory &&factory, int max_attempts,
           std::chrono::milliseconds delay) -> decltype(factory()) {
    ...
    for (int attempt = 1; attempt <= max_attempts; ++attempt) {
        try {
            ...
            std::fprintf(
                stderr,
                "[RETRY] Attempt %d/%d failed: %s. Retrying in %ldms...\n",
...
template <typename T> task<T> fallback(task<T> primary, task<T> backup) {
...
template <typename T> task<T> unwrap_or(task<T> t, T default_value) {
    try {
        co_return co_await std::move(t);
    } catch (...) {
...
template <typename T, typename From, typename To> task<T> map_error(task<T> t) {
    try {
        co_return co_await std::move(t);
    } catch (const From &e) {
        throw To(e.what());
