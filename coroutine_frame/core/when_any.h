...
task<std::pair<size_t, T>> when_any(std::vector<task<T>> &&tasks) {
    ...
    for (size_t i = 0; i < tasks.size(); ++i) {
        auto wrapper = [i, &completed, &result, &result_mutex,
                        &cv](task<T> t) -> task<void> {
            try {
                ...
                // Ignore exceptions for when_any; could be extended to
                // propagate
            }
        };
...
template <typename T> task<T> when_any_value(std::vector<task<T>> &&tasks) {
