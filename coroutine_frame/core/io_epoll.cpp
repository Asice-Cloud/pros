...
EpollReactor::~EpollReactor() { shutdown(); }
...
void EpollReactor::shutdown() {
    ...
    if (!stop_.compare_exchange_strong(expected, true))
        return;
    if (ep_ >= 0) {
        // Wake epoll_wait by closing epoll fd in destructor path is risky;
        // safer to use a timerfd Here we add a 0ms timer to wake the loop
        int tfd = timerfd_create(CLOCK_MONOTONIC, TFD_NONBLOCK | TFD_CLOEXEC);
        if (tfd >= 0) {
            itimerspec its{};
            its.it_value.tv_nsec = 1; // wake asap
...
void EpollReactor::wait_timer(std::chrono::milliseconds ms,
        ...
        for (int i = 0; i < n; ++i) {
            ...
            // Timer?
            {
                std::coroutine_handle<> h{};
                {
                    ...
                    if (it != timer_waiters_.end()) {
                        h = it->second;
                        timer_waiters_.erase(it);
                    }
                }
                if (h) {
                    uint64_t exp;
            ...
            std::coroutine_handle<> rd{}, wr{};
            {
                ...
                if (it != waiters_.end()) {
                    if (ev & (EPOLLIN | EPOLLRDHUP | EPOLLHUP | EPOLLERR)) {
                        rd = it->second.rd;
                        it->second.rd = nullptr;
                        it->second.mask &=
                            ~(EPOLLIN | EPOLLRDHUP | EPOLLHUP | EPOLLERR);
                    }
                    if (ev & (EPOLLOUT | EPOLLHUP | EPOLLERR)) {
                        wr = it->second.wr;
                        it->second.wr = nullptr;
                        it->second.mask &= ~(EPOLLOUT | EPOLLHUP | EPOLLERR);
                    }
                    if (it->second.mask)
                        arm_fd(fd, it->second.mask);
                    else
                        disarm_fd_if_idle(fd);
                }
            }
            if (rd)
                get_global_executor().schedule(rd);
            if (wr)
                get_global_executor().schedule(wr);
        }
    }
}
