...
class task_cancelled : public std::exception {
  public:
    const char *what() const noexcept override { return "Task was cancelled"; }
};
...
class cancellation_check {
  public:
    ...
    void await_resume() const { token_.throw_if_cancelled(); }
...
// Cancellable delay
inline task<void> cancellable_delay(std::chrono::milliseconds duration,
